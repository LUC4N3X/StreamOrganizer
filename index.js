...
// --- TRUST PROXY (Hugging Face/Vercel/Docker) ---
    ...
    contentSecurityPolicy: {
      directives: {
        ...
        "connect-src": [
          ...
          process.env.VERCEL_URL // Aggiunge automaticamente l'URL di Vercel
        ],
...
const allowedOrigins = [
  'https://luca12234345-stremorganizer.hf.space',
  'http://localhost:7860',
  // Aggiungi qui il tuo URL di Vercel di produzione
  // es: 'https://stremorganizer.vercel.app' 
];

// Aggiungi dinamicamente l'URL di Vercel per le preview
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // Permetti se l'origine è nella whitelist
    if (allowedOrigins.indexOf(origin) !== -1) {
      return callback(null, true);
    }
    
    // Gestione speciale per le Vercel Preview (hanno URL unici)
...
const cookieOptions = {
    httpOnly: true,
    // Su Vercel è sempre 'production' a meno che non sia una preview
    secure: true, 
    sameSite: 'strict', // 'strict' o 'lax'. Se hai problemi, prova 'lax'
...
// Forza HTTPS (Vercel lo fa comunque, ma questo gestisce 'x-forwarded-proto' se necessario)
app.use((req, res, next) => {
  // `VERCEL_ENV` è 'production' sia per prod che per preview
